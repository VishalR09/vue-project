  ...
  <div v-if="zoomEnabled" class="pan-overlay">
    ...
    <div
      ...
      :style="{ 
        ...
        width: (width - bar) + 'px',  /* prevent overlap */
        height: bar + 'px' 
      }"
    ...
    <div
      ...
      :style="{ 
        ...
        top: margin.top + 'px', 
        height: (height - bar) + 'px', /* prevent overlap */
        width: bar + 'px' 
      }"
...
const bar = 14;
const minThumb = 60; // longer thumbs
...
// Adjusted track sizes
const trackW = computed(() => width - bar);
const trackH = computed(() => height - bar);

// Longer thumbs
const thumbW = computed(() => Math.max(minThumb, (winX.value / 100) * trackW.value));
const thumbH = computed(() => Math.max(minThumb, (winY.value / 100) * trackH.value));

// Adjusted thumb positions
const thumbLeft = computed(() => (offsetX.value / (100 - winX.value)) * (trackW.value - thumbW.value));
const thumbTop = computed(() => (1 - offsetY.value / (100 - winY.value)) * (trackH.value - thumbH.value));
...
.track {
  ...
  left: 380px; top: 118px; /* ✅ kept as requested */
}
